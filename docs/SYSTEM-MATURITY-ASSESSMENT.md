...
| PSP 人格框架 | ✅ 100% | V8.1 已纳入默认人格，V8.0/V8.5 保留为可切换人格 |
...
| PSP 分析 | Mercury Lab 脚本 + LLM API + analysis_persona | 65% |
...
第二轮架构修复后，V8.0 不再被当成唯一默认人格。`analysis_persona` 已拆出，默认改为 `v8.1-reality-sync`：先解释现实均衡为何成立，再判断断裂、错配和可触达杠杆。V8.0 保留为突围型人格，V8.5 保留为纠偏审计人格。
...
| 优先级 | 问题 | 预计解决时间 |
|--------|------|-------------|
| ✅ P0 | **设计最小可用的 V8.0/PSP 执行脚本**（不依赖具体供应商/智能体探索，仍依赖 LLM API） | 已完成 |
| ✅ P0 | **拆分 analysis_persona 与 execution_mode，默认纳入 PSP-V8.1** | 已完成 |
...
- 写入 `01_segmented/`：当前 PSP 人格的结构化分析结果。
...
- 当前已把“怎么调用 PSP”和“用哪个人格判断”固化为脚本与配置，降低 OpenClaw/其他智能体的探索压力。
- 真实外部验证机制仍是下一阶段问题。
...
- PSP 人格提示词：`v8-mercury-backend/docs/methods/`
